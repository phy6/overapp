...
const WORKSPACE_ROOTS = [
    '/home/martin/Code/dublin-opendata',
    '/home/martin/Code/The_Activist3',
    '/home/martin/Code/market-watch',
    '/home/martin/Code/OverApp',
    '/home/martin/Code/test-init',
    '/home/martin/Code/test-project'
];
...
async function generateProjectFiles(name, description, projectTitle, initialPrompt) {
  const root = path.join('/home/martin/Code', name);
  try {
    if (!fs.existsSync(root)) fs.mkdirSync(root, { recursive: true });
    const title = projectTitle || name.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
    const desc = initialPrompt || description || `${title} is a new seeded project.`;
    const contextLines = `# ${title} — Domain Glossary\n\n## Project Description\n${desc}\n\n## Core Terms\n\n### ${title}\nA project focused on ${desc}.\n\n## Core Operations\n\n1. **List Items** — Returns all available items\n2. **Create Item** — Creates a new item\n3. **Search Items** — Searches items by keyword\n\n## Open Questions\n\n1. What is the primary data source?\n2. What authentication model is needed?\n3. What is the target deployment environment?\n`;
